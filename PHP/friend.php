        ...
        <div class="col-md-5 whiteBackground">
            ...
            <?php
               ...
               while($results = mysqli_fetch_array($result)){
				...
				data-whatever='".$friresults['Username']."' data-sendto='".$friresults['Username']."'>Send Message</button>
...
<!--
//          $result = findNotFriend($link,$_SESSION['id'],$_SESSION['teamid']);
//			$result = mysqli_query($link, "SELECT * FROM `Fansclub` where FTeamId=".$_SESSION['teamid']);
//          $result = mysqli_query($link, "SELECT * FROM `Fansclub` where FUserID=".$_SESSION['id']);
						
/*			$results = mysqli_fetch_array($result);
			$row=$results['FTeamID'];
				$get ="SELECT UserID, UserName, Address FROM `Fansclub` NATURAL JOIN `User` where FTeamID='".$row."'";
				$getresult = mysqli_query($link,$get);
				while($getresults = mysqli_fetch_array($getresult)) {
			echo "<div>
                <h4>".$getresults['UserID']."</h4>
...
/*			while($results = mysqli_fetch_array($result)) {
				echo "<div>
                <h4>".$results['FUserID']."</h4>
                <h4>".$results['FTeamID']."</h4>
...
-->
        <div class="col-md-5 whiteBackground">
			...
			while($results = mysqli_fetch_array($result)) {
				...
				<h4>Address: ".$getresults['Address']."</h4>	
...
/*				//show multiple matched teamid. success
				echo "<div>
                <h4>".$row."</h4>
                <button name=\"new\" type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"window.location.href='friend.php?friendid={$results['Userid']}'\">Add</button>
            ...
            <form method="post" >
                <div class="modal-body">
                    <div class="form-group">
                        <label>To</label>
                        <input class="form-control" type="text" name="sendto" id="sendto" disabled/>
                    </div>
                    ...
                    <div class="form-group">
                        <label>Content</label>
                        <textarea class="form-control" name="sendcontent" rows="10" cols="80" class="center-block" placeholder="Write something here"></textarea>
                    </div>
                </div>
